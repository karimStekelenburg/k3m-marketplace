...
with mean, stddev, min, max for each metric.
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines.append(f"| Pass Rate | {a_pr.get('mean', 0)*100:.0f}% | {b_pr.get('mean', 0)*100:.0f}% | {delta.get('pass_rate', '-')} |")

    a_time = a_summary.get("time_seconds", {})
    b_time = b_summary.get("time_seconds", {})
    lines.append(f"| Time | {a_time.get('mean', 0):.1f}s | {b_time.get('mean', 0):.1f}s | {delta.get('time_seconds', '-')}s |")
