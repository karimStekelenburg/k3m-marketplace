...
description: >-
  ...
  /meta-dev:report-issue. Performs root-cause analysis, patches plugin-dev,
  checks if meta-dev caused the issue, patches meta-dev if needed, validates,
...
Diagnose and fix plugin-dev issues reported by users. Determine if meta-dev's
...
If a `meta-dev/sync-*` commit introduced the issue:
- The meta-dev pipeline has a bug
- Identify which agent failed (discovery-crawler missed it? diff-reviewer approved bad change?)
- Prepare a fix for the relevant meta-dev component

### 4. Patch Plugin-Dev

1. Create branch: `meta-dev/fix-<component>-<short-desc>`
2. Apply the fix to plugin-dev
3. Validate: `claude plugin validate ../plugin-dev`

### 5. Patch Meta-Dev (if applicable)

If meta-dev caused the issue, also fix the relevant meta-dev file:
...
Commit all changes (both plugin-dev and meta-dev if applicable) and create a PR.
...
### Prevention
[What meta-dev improvement prevents this class of issue in the future]
```
...
- The meta-dev fix is as important as the plugin-dev fix; preventing recurrence matters
