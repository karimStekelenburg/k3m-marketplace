...
Run `/marketplace-maintenance:update-refs {plugin}` to update stale references.
```
...
This command is **read-only and diagnostic**. It never modifies files. It tells you what might be stale and recommends next steps. The actual updating happens via `/marketplace-maintenance:update-refs`.
